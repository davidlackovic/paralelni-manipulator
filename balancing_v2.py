...
com = communicator_v2.SerialCommunication(ser=ser, normal_acceleration=150)  # communication object iz communicator_v2
feedrate = 3000
...
while True:
    ...
    if np.all(current_position != None):
        ...
        if np.linalg.norm(PID.current_error) < 150:
            new_angle += (1-(PID.current_error/150))*0.0032*np.sin(5*2*np.pi*time.time())
        print(new_angle)
        ...
        if np.all(calculated_angles!=0):
            ...
            if time.time_ns() - old_move_time > 2*time_to_move*1e9:
