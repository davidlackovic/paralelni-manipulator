...
p_0 = np.array([WIDTH/2, HEIGHT/2]) #zacetna lega
...
def pid_controller(current_postition=np.ndarray):
    ...
    max_change = max_change_rate * time_delta
...
def main():
     ...
     while True:
        ...
        if time.time()-old_time > 0.035:
            ...
            vx, vy = v_0
            noise_value = 1
            x_rand = x + np.random.random_integers(-1, 1)/noise_value
            y_rand = y + np.random.random_integers(-1, 1)/noise_value
            loss_factor = 0.995
            vx_rand = vx * loss_factor
            vy_rand = vy * loss_factor

            #print(f'[{time.strftime('%Y-%m-%d %H:%M:%S')}.{int(time.time() * 1000) % 1000:03d}]', p_0, v_0, x_rand, y_rand)
            p_0 = np.array([x_rand, y_rand])
            v_0 = np.array([vx_rand, vy_rand])
