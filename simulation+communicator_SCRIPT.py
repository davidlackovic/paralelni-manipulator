...
com = communicator_v2.SerialCommunication(ser=ser)  # communication object iz communicator_v2
...
def main():
     ...
     zacetni_polozaj = kinematika.izracun_kotov(b=b, p=p, l_1=l_1, l_2=l_2, h=0.3, psi_x_deg=0, psi_y_deg=0)
     pozicije_motorjev = kinematika.deg2steps(zacetni_polozaj)
     com.move_to_position(pozicije_motorjev, feedrate=1500)
     old_time = time.time()
     while True:
        ...
        if new_time - old_time > 0.02:
            nakloni = kinematika.izracun_kotov(b=b, p=p, l_1=l_1, l_2=l_2, h=0.3, psi_x_deg=np.rad2deg(tilt_angle[0]), psi_y_deg=np.rad2deg(tilt_angle[1]))
            pozicije_motorjev = kinematika.deg2steps(nakloni)
            com.move_to_position(pozicije_motorjev, feedrate=1500)
            old_time = new_time
