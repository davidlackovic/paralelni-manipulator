...
start_config = np.array([0.33, 0, 0])
...
#PID konstante za oranzno - za majhen error
K_p = 0.00025
K_i = 0.00035
K_d = 0.0005
...
com = communicator_v2.SerialCommunication(ser=ser, normal_acceleration=200)  # communication object iz communicator_v2
feedrate = 3000
...
com.enable_steppers()
time.sleep(1)
com.move_to_position(starting_steps, feedrate=600)
time.sleep(5)
...
if np.any(current_position==None):
    print('No contour detected on startup, exiting.')
    starting_position=kinematika.izracun_kotov(b, p, l_1, l_2, 0.255, 0, 0)
    starting_steps=kinematika.deg2steps(starting_position)
    com.move_to_position(starting_steps)
    time.sleep(4)
...
while True:
    ...
    processed_frame, current_position, current_velocity = CV.process_frame(frame, lower_color, upper_color, alpha)
    print(f'2 --- curent_posisiton: {current_position}, target_pos: {CV.target_pos}')
    ...
    if np.all(current_position != None):
        ...
        if np.linalg.norm(PID.current_error) < 150:
            new_angle += (1-(PID.current_error/150))*0.004*np.sin(5*2*np.pi*time.time())
        print(new_angle)
        ...
        print(PID.current_error, PID.K_p, PID.K_i, PID.K_d)
        ...
        if np.all(calculated_angles!=0):
            ...
            limited_steps = kinematika.limit_steps(calculated_steps, 1, 23)    # samo kot safety measure, drgac nima veze
            ...
            print(f' delta t:{(time.time_ns() - old_move_time)*1e-6} ms', f'time to move: {time_to_move*1e3} ms' )


            if time.time_ns() - old_move_time > 1.7*time_to_move*1e9:
        ...
        print(f'[{time.strftime('%Y-%m-%d %H:%M:%S')}.{int(time.time() * 1000) % 1000:03d}], {current_position}, {CV.target_pos}, {np.rad2deg(clipped_new_angle)}, {calculated_angles}')
    else:
